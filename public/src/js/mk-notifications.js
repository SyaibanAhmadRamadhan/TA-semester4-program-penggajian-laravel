...
function mkNotifications(mkConfig) {
    if (mkConfig == undefined) {
        ...
        config = {
            positionX:
                mkConfig.positionX != undefined ? mkConfig.positionX : "right",
            positionY:
                mkConfig.positionY != undefined ? mkConfig.positionY : "bottom",
            scrollable:
                mkConfig.scrollable != undefined ? mkConfig.scrollable : true,
    ...
    var mkNotifications =
            '<div id="mk-notifications" data-max="' +
            config.max +
            '" class="mk-notifications mk-' +
            config.positionY +
            " mk-" +
            config.positionX +
            scrollable +
...
function mkNoti(title, message, mkOptions) {
    ...
    if (mkOptions !== undefined) {
        var config = {
            status:
                mkOptions.status != undefined ? mkOptions.status : "default",
            icon:
                mkOptions.icon == undefined
                    ? config.icon
                    : {
                          class:
                              mkOptions.icon.class != undefined
                                  ? mkOptions.icon.class
                                  : null,
                          color:
                              mkOptions.icon.color != undefined
                                  ? mkOptions.icon.color
                                  : null,
                          background:
                              mkOptions.icon.background != undefined
                                  ? mkOptions.icon.background
                                  : null,
                      },
            link:
                mkOptions.link == undefined
                    ? config.link
                    : {
                          url:
                              mkOptions.link.url != undefined
                                  ? mkOptions.link.url
                                  : null,
                          target:
                              mkOptions.link.target != undefined
                                  ? mkOptions.link.target
                                  : "_self",
                          function:
                              mkOptions.link.function != undefined
                                  ? mkOptions.link.function
                                  : null,
                      },
            dismissable:
                mkOptions.dismissable != undefined
                    ? mkOptions.dismissable
                    : true,
            callback:
                mkOptions.callback != undefined ? mkOptions.callback : null,
            sound: mkOptions.sound != undefined ? mkOptions.sound : false,
            customSound:
                mkOptions.customSound == undefined
                    ? config.customSound
                    : {
                          onShow:
                              mkOptions.customSound.onShow != undefined
                                  ? mkOptions.customSound.onShow
                                  : defaultSound,
                          onClose:
                              mkOptions.customSound.onClose != undefined
                                  ? mkOptions.customSound.onClose
                                  : defaultClose,
                      },
            duration:
                mkOptions.duration != undefined ? mkOptions.duration : 7000,
        };
    }
    ...
    if (title != undefined) {
        ...
        totalLength = totalLength + titleLength * 1.1;
    }
    if (titleLength >= 70) {
    ...
    if (message != undefined) {
        ...
        totalLength = totalLength + messageLength;
    }
    if (totalLength >= 140) {
        message = "";
    } else if (messageLength >= 140 - titleLength * 1.1) {
        message = message.substr(0, 140 - titleLength * 1.1) + "...";
    }
            ...
            setTimeout(function () {
                ...
                if (mkNotiCount < mkMax || mkMax == "null") {
                    ...
                    for (var i = 0; i < forShow; i++) {
                        var firstPending =
                            document.getElementsByClassName("mk-pending")[
                                document.getElementsByClassName("mk-pending")
                                    .length - 1
                            ];
